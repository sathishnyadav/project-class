...
function UserLocation() {
    ...
    let [address_type, setaddresstype] = useState("")

    let user = JSON.parse(localStorage.getItem("User"))

    let data = {landMark,buildingName,house_number,city,state,country,pincode,address_type}
    
    
    let addAdress = (e) =>{
        e.preventDefault();
        axios.post(`http://localhost:8080/address/${user.id}`,data)
        .then((res)=>{
            console.log(res);
    ...
    return (
        <div className='UserLocation'>
            <form onSubmit={addAdress} action="">
                <label htmlFor="">Adress Type : </label>
                <select value={address_type} onChange={(e)=>{setaddresstype(e.target.value)}} name="" id="">
                    <option >Home</option>
                ...
                <label htmlFor="">Houseno </label><input type="text" placeholder='Enter the Houseno/floor' required value={house_number} onChange={(e) => { sethousenumber(e.target.value) }} />
